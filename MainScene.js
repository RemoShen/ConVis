...
export function createMainScene() {
  ...
  // http://localhost:5173/dataset_360_chunk.zarr/huge_chunk/.zarray

  // === 基本路径和全局变量 ===
  const basePath = "http://localhost:5173/dataset_360_chunk.zarr/";
  const allModelMap = new Map(); // key: neuron_id, value: 2D model
  let neuronIdList = null;
  let isPreloaded = false;
  let zoomThresholds = [];

  // === 加载 Zarr 数据并构建模型 ===
  async function loadDatasetAndBuildModels() {
    const group = await openGroup(basePath);
    const attrs = await group.attrs.asObject();

    neuronIdList = attrs["neuron_ids"]; // e.g. ["7205759...", ...]
    const array = await openArray({ store: group.store, path: "huge_chunk" });
    const data = await array.get();
    const chunk = data.data; // 3D: [N_neurons, N_nodes, 7]

    for (let i = 0; i < chunk.length; i++) {
      const neuron_id = neuronIdList[i];
      const neuronArray = chunk[i];

      // 过滤有效行 + 转成对象数组
      const isZeroRow = (row) => row.every((val) => val === 0);

      const nodes = Array.from(neuronArray)
      ...
      model.visible = false;
      model.userData.neuronId = neuron_id;

      if (i == 11) {
        console.log(model); // 没问题
      }

      allModelMap.set(neuron_id, model);
      scene.add(model);
      console.log(`Added model for ${neuron_id}, nodes: ${nodes.length}, visible: ${model.visible}`);
    }
  }
  ...
  async function preloadAllModels() {
    ...
    await loadDatasetAndBuildModels(); // 👈 核心：从 Zarr 中构建模型
  ...
  function showOnlyLevel(levelName) {
    ...
    if (Array.isArray(hoverList)) {
      const hoverIds = hoverList.map(extractId);
      visibleSet = new Set(levelIds.filter((id) => hoverIds.includes(id)));
    } else {
      visibleSet = new Set(levelIds);
    }
